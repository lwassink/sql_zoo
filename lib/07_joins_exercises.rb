...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    FROM
      actors AS lead_actors JOIN castings AS lead_castings
        ON lead_actors.id = lead_castings.actor_id
      JOIN movies
        ON movies.id = lead_castings.movie_id
      JOIN castings AS julie_castings
        ON julie_castings.movie_id = movies.id
      JOIN actors AS julie_actors
        ON julie_actors.id = julie_castings.actor_id
    WHERE
      lead_castings.ord = 1
      AND
...
def colleagues_of_garfunkel
  ...
  SELECT
    other_actors.name
  FROM
    actors AS other_actors JOIN castings AS other_castings
      ON other_actors.id = other_castings.actor_id
    JOIN castings AS g_castings
      ON g_castings.movie_id = other_castings.movie_id
    JOIN actors AS g_actors
      ON g_actors.id = g_castings.actor_id
  WHERE
    g_actors.name = 'Art Garfunkel'
